...
public class STTUtilsWebSocket {
    ...
    // private static final AudioFormat AUDIO_FORMAT = new AudioFormat(SAMPLE_RATE,
    // SAMPLE_SIZE_BITS, CHANNELS, true,
    // false);

    // // chunk format (50ms audio, will be amount of audio sent per websocket
    // event)
    // private static final int CHUNK_MS = 50;
    // private static final int FRAME_SIZE = AUDIO_FORMAT.getFrameSize(); // bytes
    // per frame (2 for 16 bit mono)
    // private static final int SAMPLES_PER_CHUNK = (int) (SAMPLE_RATE * (CHUNK_MS /
    // 1000.0));
    ...
    // private static final int QUEUE_CAPACITY = 256; // tune for your app: 256 *
    // 1600 ≈ 400 KB
    // private static final BlockingQueue<byte[]> CHUNK_QUEUE = new
    // LinkedBlockingQueue<>(QUEUE_CAPACITY);

    // // executor for sending chunks asynchronously
    // private static final ExecutorService SENDER_POOL =
    ...
    // byte[] rawReadBuffer = new byte[4096]; // at 16000 sample rate, 16 bit mono,
    // 2 bytes per sample => 32k bytes/
    ...
    // System.arraycopy(rawReadBuffer, offset, accumulateBuffer50ms, accumulatePos,
    // numBytesToCopy);
    ...
    // wsutils.connect(new
    // URI("wss://api.player2.game/v1/stt/stream?sample_rate=16000&encoding=wav"),
    // token);
