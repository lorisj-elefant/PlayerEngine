...
public class Prompts {
  ...
  public static void addCommandToBuilder(StringBuilder builder, String name, String description) {
    int padSize = 10;
    builder.append(name).append(": ");
    int toAdd = padSize - name.length();
    builder.append(" ".repeat(Math.max(0, toAdd)));
  ...
  public static String getAINPCSystemPrompt(Character character, Collection<Command> altoclefCommands,
    ...
    for (Command c : altoclefCommands) {
      // for each command this will give something like "commandName: "
      StringBuilder line = new StringBuilder();
      addCommandToBuilder(commandListBuilder, c.getName(), c.getDescription());
      commandListBuilder.append(line);
    }

    for (PseudoCommands.PseudoCommand c : PseudoCommands.pseudoCommands) {
      StringBuilder line = new StringBuilder();
      addCommandToBuilder(commandListBuilder, c.getName(), c.getDescription());
      commandListBuilder.append(line);
    }
  ...
  private static String buildStructurePrompt = """
