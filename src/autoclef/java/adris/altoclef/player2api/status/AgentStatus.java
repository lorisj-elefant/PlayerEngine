...
public class AgentStatus extends ObjectStatus {
  public static adris.altoclef.player2api.status.AgentStatus fromMod(AltoClefController mod) {
    ...
    return (adris.altoclef.player2api.status.AgentStatus)(new adris.altoclef.player2api.status.AgentStatus())
      .add("health", String.format("%.2f/20", Float.valueOf(player.getHealth()))).add("food", String.format("%.2f/20", Float.valueOf(mod.getBaritone().getEntityContext().hungerManager().getFoodLevel()))).add("saturation", String.format("%.2f/20", Float.valueOf(mod.getBaritone().getEntityContext().hungerManager().getSaturationLevel()))).add("inventory", StatusUtils.getInventoryString(mod))
