...
public class FollowPlayerTask extends Task {
  ...
  protected Task onTick() {
    ...
    Optional<Vec3d> lastPos = mod.getEntityTracker().getPlayerMostRecentPosition(this._playerName);
    if (lastPos.isEmpty()) {
      setDebugState("No player found/detected. Doing nothing until player loads into render distance.");
      return null;
    } 
    Vec3d target = lastPos.get();
    if (target.isInRange((Position)mod.getPlayer().getPos(), 1.0D) && !mod.getEntityTracker().isPlayerLoaded(this._playerName)) {
      mod.logWarning("Failed to get to player \"" + this._playerName + "\". We moved to where we last saw them but now have no idea where they are.");
      stop();
      return null;
    } 
    Optional<PlayerEntity> player = mod.getEntityTracker().getPlayerEntity(this._playerName);
    if (player.isEmpty())
      return (Task)new GetToBlockTask(new BlockPos((int)target.x, (int)target.y, (int)target.z), false); 
    return (Task)new GetToEntityTask((Entity)player.get(), this._followDistance);
  }
  ...
  protected boolean isEqual(Task other) {
    if (other instanceof adris.altoclef.tasks.movement.FollowPlayerTask) {
      ...
      return (task._playerName.equals(this._playerName) && Math.abs(this._followDistance - task._followDistance) < 0.1D);
    } 
    return false;
  }
  
  protected String toDebugString() {
    return "Going to player " + this._playerName;
  }
}
