...
public abstract class ResourceTask extends Task implements ITaskCanForce {
  ...
  protected Task onTick() {
    ...
    if (!shouldAvoidPickingUp(controller) && controller.getEntityTracker().itemDropped(_itemTargets)) {
      if (isPickupTaskValid(controller)) {
        setDebugState("Picking up dropped items");
        return _pickupTask;
      }
    }
