...
public class PlayerInteractionFixChain extends TaskChain {
  private final TimerGame stackHeldTimeout = new TimerGame(1.0D);
  
  private final TimerGame generalDuctTapeSwapTimeout = new TimerGame(30.0D);
  
  private final TimerGame shiftDepressTimeout = new TimerGame(10.0D);
  
  private final TimerGame betterToolTimer = new TimerGame(0.0D);
  
  private final TimerGame mouseMovingButScreenOpenTimeout = new TimerGame(1.0D);
  ...
  public float getPriority() {
    ...
    if (mod.getUserTaskChain().isActive() && this.betterToolTimer.elapsed()) {
      ...
      if (mod.getControllerExtras().isBreakingBlock()) {
        ...
        Slot currentEquipped = PlayerSlot.getEquipSlot(controller.getInventory());
        if (bestToolSlot.isPresent() && !((Slot)bestToolSlot.get()).equals(currentEquipped))
          if (StorageHelper.getItemStackInSlot(currentEquipped).getItem() != StorageHelper.getItemStackInSlot(bestToolSlot.get()).getItem()) {
            boolean isAllowedToManage = ((!mod.getBaritone().getPathingBehavior().isPathing() || ((Slot)bestToolSlot.get()).getInventorySlot() >= 9) && !mod.getFoodChain().isTryingToEat());
    ...
    if (this.generalDuctTapeSwapTimeout.elapsed() && 
    ...
    ItemStack currentStack = StorageHelper.getItemStackInCursorSlot(controller);
    if (currentStack != null && !currentStack.isEmpty()) {
      if (this.lastHandStack == null || !ItemStack.areEqual(currentStack, this.lastHandStack)) {
    ...
    if (this.lastHandStack != null && this.stackHeldTimeout.elapsed()) {
      ...
      if (ItemHelper.canThrowAwayStack(mod, StorageHelper.getItemStackInCursorSlot(controller))) {
