...
public class AICommandBridge {
   ...
   public void onChatMessage(ServerChatEvent evt) {
      String message = evt.getMessage().getString();
      String sender = evt.getUsername();
      ...
      if (distance > 200.0F) {
         ...
         System.out.printf("[AIBridge/CharMessageEvent]/MESSAGE (%s) SENDER (%s), DISTANCE(%.2f%n)", message, sender, distance);
         if (sender != null && !Objects.equals(sender, receiver)) {
            String wholeMessage = "Other players: [" + sender + "] " + message;
            this.addMessageToQueue(wholeMessage + "| Remember to roleplay as your character");
         }
      }
   }
   ...
   private void updateInfo(Character character) {
      ...
      for (Command c : this.mod.getCommandExecutor().allCommands()) {
      ...
      if (this.conversationHistory == null) {
         this.conversationHistory = new ConversationHistory(this.player2GameId, newPrompt, this.character.name, this.character.shortName);
      } else {
      ...
      llmThread.submit(
         () -> {
            try {
               ...
               JsonObject response = Player2APIService.completeConversation(this.player2GameId, historyWithStatus);
               ...
               if (llmMessage != null && !llmMessage.isEmpty()) {
                  this.mod
                     ...
                     .getServer()
                     .getPlayerList()
                     .broadcastSystemMessage(Component.nullToEmpty("<" + this.character.shortName + "> " + llmMessage), false);
                  this.mod.logCharacterMessage(llmMessage, this.character, true);
                  Player2APIService.textToSpeech(this.player2GameId, llmMessage, this.character);
               }

               String commandResponse = Utils.getStringJsonSafely(response, "command");
               if (commandResponse != null && !commandResponse.isEmpty()) {
                  String commandWithPrefix = this.cmdExecutor.isClientCommand(commandResponse)
                     ? commandResponse
                     : this.cmdExecutor.getCommandPrefix() + commandResponse;
                  ...
                  this.cmdExecutor
                     .execute(
                        commandWithPrefix,
                        () -> {
                           if (this.mod.isStopping) {
                              System.out.printf("[AICommandBridge/processChat]: (%s) was cancelled. Not adding finish event to queue.", commandWithPrefix);
                           }

                           if (this.messageQueue.isEmpty() && !this.mod.isStopping) {
                              this.addMessageToQueue(
                                 String.format(
                                    "Command feedback: %s finished running. What shall we do next? If no new action is needed to finish user's request, generate empty command `\"\"`.| Remember to roleplay as "
                                       + this.character.name,
                                    commandResponse
                                 )
                              );
                        ...
                        err -> this.addMessageToQueue(
                           String.format(
                              "Command feedback: %s FAILED. The error was %s.| Remember to roleplay as " + this.character.name,
                              commandResponse,
                              err.getMessage()
                           )
                        )
      ...
      llmThread.submit(
         () -> {
            ...
            if (this.conversationHistory.isLoadedFromFile()) {
               this.addMessageToQueue(
                  "You want to welcome user back. IMPORTANT: SINCE THIS IS THE FIRST MESSAGE, DO NOT SEND A COMMAND!!| Remember to roleplay as "
                     + this.character.name
               );
            } else {
               this.addMessageToQueue(
                  this.character.greetingInfo
                     + " IMPORTANT: SINCE THIS IS THE FIRST MESSAGE, DO NOT SEND A COMMAND!!| Remember to roleplay as "
                     + this.character.greetingInfo
               );
            }
         }
